...
//  HAL_UART_Transmit(&huart2, (uint8_t*)"DEBUG: BLE Init OK\r\n", 21, 1000);
...
#else
  /* 1. MAX30101 检测 - PPG功能已禁用 */
  HAL_UART_Transmit(&huart2, (uint8_t*)"DEBUG: PPG Disabled (ENABLE_PPG=0)\r\n", 41, 1000);
#endif
...
#endif
#else
  /* 3. MAX30101 初始化 - PPG功能已禁用 */
  HAL_UART_Transmit(&huart2, (uint8_t*)"DEBUG: PPG Init skipped (ENABLE_PPG=0)\r\n", 45, 1000);
#endif
  ...
  while (1)
  {
    ...
    if(ADC_1to4Voltage_flag == 4){
      ...
      // --- 2. 打包 ACC ---
      // 注意: 发送是高位在前，传感器先发低后发高 -> 高位取 1, 3, 5
...
#else
      /* --- PPG 功能禁用：数据补0 --- */
      allData[PPG_START_INDEX + 0] = 0x00;
