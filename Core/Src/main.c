...
//  #endif
  HAL_UART_Transmit(&huart2, (uint8_t*)"DEBUG: PPG Disabled (IIC suspended)\r\n", 36, 1000);
  ...
  while (1)
  {
    ...
    if(ADC_1to4Voltage_flag == 4){
      ...
      // --- 3. PPG 数据处理 - 禁用IIC，数据补0 ---
      // PPG 数据位置 (allData[13-16]) 全部补0，保持传输位数不变
